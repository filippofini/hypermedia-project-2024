...
export default defineEventHandler(async (event) => {
    ...
    const { data: serviceData, error: serviceError } = await client.from('Service').select("id_service, description, image, small_desc, title, schedule").eq('id_service', id).single();
    if (serviceError) {
    ...
    // Fetch team details by joining People and Works_Service tables
     // Fetch team details by joining People and Works_Service tables
  const { data: teamData, error: teamError } = await client.from('Works_Service').select(`is_supervisor,People (id, name, surname)`).eq('id_service', id);
